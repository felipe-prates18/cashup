...
REQUIRED_TABLES = ("transactions", "titles", "reconciliation_items")
...
def validate_required_tables(cursor: sqlite3.Cursor) -> None:
    missing_tables = [table_name for table_name in REQUIRED_TABLES if not table_exists(cursor, table_name)]
    ...
    available_list = ", ".join(available_tables) if available_tables else "nenhuma"
    missing_list = ", ".join(missing_tables)
    print(
        "Falha ao limpar lançamentos: tabelas obrigatórias ausentes "
        f"({missing_list}). Tabelas encontradas: {available_list}.",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
def confirm(force: bool, db_path: Path, transaction_count: int) -> None:
    ...
    print(f"Tabela de lançamentos validada: transactions")
...
def reset_transaction_sequence(cursor: sqlite3.Cursor) -> None:
...
def run_cleanup(db_path: Path, dry_run: bool) -> dict[str, int]:
    ...
    try:
        ...
        validate_required_tables(cursor)
        ...
        reset_transaction_sequence(cursor)
        connection.commit()
        return metrics
...
def main() -> None:
    ...
    print(f"- tabela validada: transactions")
